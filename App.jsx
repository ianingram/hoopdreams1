...
const CreateChallengeModal = ({ onClose, showToast }) => {
  const [tab, setTab] = useState('skill'); // 'standard' or 'skill'
  const [challengeType, setChallengeType] = useState('3-Point Shootout');
  const [stake, setStake] = useState("500");
  const [currency, setCurrency] = useState("HD");

  const handleSubmit = () => {
    showToast(`New ${challengeType} challenge created for ${stake} ${currency}!`);
    onClose();
  };
      ...
      <div className="bg-slate-900 border border-slate-700 rounded-3xl w-full max-w-md shadow-2xl overflow-hidden transform animate-in zoom-in-95">
        ...
        <div className="p-6">
          <div className="flex bg-slate-800 rounded-xl p-1 mb-6">
            <button onClick={() => setTab('standard')} className={`flex-1 py-2 rounded-lg text-sm font-bold transition-all ${tab === 'standard' ? 'bg-slate-700 text-white shadow-sm' : 'text-slate-500 hover:text-slate-300'}`}>Standard Match</button>
          ...
          {tab === 'skill' && (
            <div className="space-y-4 mb-8">
              <div>
                <label className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-2 block">Challenge Type</label>
                <select 
                ...
                >
                  ...
                  <option>1v1 King of the Court</option>
                </select>
              </div>
          ...
          {tab === 'standard' && (
             <div className="mb-8 text-center py-8 text-slate-400">
               ...
               <p>Standard matchmaking configuration.</p>
             </div>
          )}
    ...
    <div className="p-4 md:p-8 space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-7xl mx-auto relative">
      <div className="flex justify-between items-center bg-slate-900 border border-slate-800 p-4 rounded-2xl shadow-sm">
        <div className="flex items-center gap-3">
          <div className="bg-orange-500/20 p-2 rounded-lg"><MapPin size={20} className="text-orange-500" /></div>
          <div>
            <p className="text-xs text-slate-400 font-bold uppercase tracking-wider">Current Zone</p>
            <p className="text-white font-semibold text-sm md:text-base">New York County, NY</p>
          </div>
        </div>
      </div>
